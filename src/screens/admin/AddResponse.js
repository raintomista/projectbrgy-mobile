...
import { addResponse } from 'services/ReportService';
...
export default class AddResponse extends Component {
  ...
  async handleSubmit() {
    ...
    Keyboard.dismiss()
    try {
      addResponse(this.reportId, formData);
      ToastAndroid.show(localized.RESPOND_SUCCESS, ToastAndroid.SHORT);    
      this.toggleDisable();
      this.input.focus();
      NavigationService.dispatch(StackActions.popToTop());
      NavigationService.push('ReportOverview', { reportId: this.reportId })         
    } catch(e) {        
  ...
  render() {
    const charCount = 800 - this.message.length;
    return (
      <Container>
        <HeaderWithGoBack title="Add Response" />
  ...
  checkDisabled(charCount) {
    return this.disabled || charCount === 800 || charCount < 0;
  }
}
