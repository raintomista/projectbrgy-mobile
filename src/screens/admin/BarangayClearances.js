...
import { getAllClearances } from 'services/EService';
...
@observer
export default class BarangayClearances extends Component {
  @observable page = 0;
  @observable limit = 20;
  @observable hasMore = true;

  @observable tableHeader = ['Applicant Name', 'Citizenship', ' Purpose', 'Request Date'];
  @observable tableWidth = [200, 200, 220, 200];
  @observable tableData = null;

  @action
  async getClearances(brgyId) {
    this.page += 1;
    try {
      const response = await getAllClearances(brgyId, this.page, this.limit);  
      if(response.data.data.barangay_clearance.length > 0) {
        runInAction(() => {
          if(!this.tableData) {
            this.tableData = response.data.data.barangay_clearance;
          } else {
  ...
  renderItem = ({item, index}) => {
    const rowData = [];
    rowData.push(this.renderCell(`${item.first_name} ${item.last_name}`, item.status));
    rowData.push(this.renderCell(item.citizenship, item.status));
    rowData.push(this.renderCell(item.purpose, item.status));    
  ...
  render() {
    return (
      <Container>
        <HeaderWithDrawer title="Barangay Clearance Requests" />
  ...
  handlePress(item, index) {
    ...
    NavigationService.push('ClearanceOverview', { inquiryId: item.id});
  }
}
